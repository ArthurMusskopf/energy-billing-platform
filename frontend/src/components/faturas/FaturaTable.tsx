...
export function FaturaTable({
  ...
  const toggleRow = (id: string) => {
    ...
    const willOpen = !newExpanded.has(id);
                ...
                <>
                  <CollapsibleTrigger asChild>
                    ...
                    >
                      ...
                      <TableCell>{getStatusBadge(fatura.status, fatura.alertas.length)}</TableCell>
                      <TableCell className="text-right">
                        <Button
                          size="sm"
                          variant={fatura.status === "validado" ? "secondary" : "default"}
                          disabled={fatura.status === "validado" || isValidating}
                          onClick={(e) => {
                            e.stopPropagation();
                        ...
                        <div className="p-6 space-y-4">
                          ...
                          <div className="grid grid-cols-4 gap-4 text-sm">
                            ...
                            <div>
                              ...
                              <p className="font-medium">{fatura.dias || 0}</p>
                            </div>
                            <div>
                                    ...
                                    <TableRow key={item.id}>
                                      ...
                                      <TableCell className="text-right">
                                        <EditableCell
                                          ...
                                          isEditing={
                                            editingCell?.faturaId === fatura.id &&
                                            editingCell?.field === `quantidade-${item.id}`
                                          }
                                          editValue={editValue}
                                          onEdit={() =>
                                            startEdit(fatura.id, `quantidade-${item.id}`, item.quantidade)
                                          }
                                          onSave={saveEdit}
                                          onCancel={cancelEdit}
                                      ...
                                      <TableCell className="text-right">
                                        {formatCurrency(item.pis_valor)}
                                      </TableCell>
                                      <TableCell className="text-right">{item.icms_aliquota}%</TableCell>
                                      <TableCell className="text-right">
                                        {formatCurrency(item.icms_valor)}
                                      </TableCell>
                                    </TableRow>
                                  ))}
                                </TableBody>
