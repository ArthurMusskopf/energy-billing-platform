...
def load_reporting_fact(
    ...
    if cliente_numero:
        where_parts.append(
            "COALESCE(wf.cliente_numero, it.cliente_numero, b.cliente_numero, cli.cliente_numero) = @cliente_numero"
        )
        params["cliente_numero"] = str(cliente_numero)

    if unidade_consumidora:
        where_parts.append(
            "COALESCE(wf.unidade_consumidora, it.unidade_consumidora, b.unidade_consumidora, cli.unidade_consumidora) = @uc"
        )
    ...
    WITH itens_nf AS (
      SELECT
        ...
        ANY_VALUE(serie) AS serie_fatura,
        ANY_VALUE(leitura_anterior) AS leitura_anterior,
        ANY_VALUE(leitura_atual) AS leitura_atual,
        ANY_VALUE(dias) AS dias_faturados,
        ANY_VALUE(proxima_leitura) AS proxima_leitura,
    ...
    SELECT
      ...
      COALESCE(it.serie_fatura, '') AS serie_fatura,
...
def load_report_header(nota_fiscal: str) -> pd.DataFrame:
    """
...
def build_report_payload(nota_fiscal: str) -> Dict[str, Any]:
    """
    Payload único para a futura geração do demonstrativo individual.
    """
