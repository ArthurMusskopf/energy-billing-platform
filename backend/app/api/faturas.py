...
def _persist_parse_outputs(lote: Dict[str, Any], workflow_df: pd.DataFrame) -> Dict[str, Any]:
    result: Dict[str, Any] = {}
    ...
    for name, df, table in [
        ...
        if df is None or df.empty:
            result[name] = {"ok": True, "table": table, "affected_rows": 0}
            continue

        try:
            affected = upsert_dataframe(df, table, key_column="id")
            result[name] = {"ok": True, "table": table, "affected_rows": int(affected)}
...
def _build_cadastro_cliente_payload(
    ...
    uc_cadastrada = bool(client_row) and bool(_safe_str(client_row.get("unidade_consumidora")))
    n_fases = _safe_int(
        _coalesce_value(
    ...
    custo_disp = _safe_float(
        _coalesce_value(client_row.get("custo_disp"), workflow_row.get("custo_disp"), compute_custo_disp(n_fases)),
        default=None,
    )
    ...
    cadastro = {
        ...
        "nome": _safe_str(
            _coalesce_value(client_row.get("nome"), workflow_row.get("nome"), _first_non_null(itens_df, "nome"))
        ),
        "cnpj_cpf": _safe_str(
            _coalesce_value(
        ...
        "cep": _safe_str(
            _coalesce_value(client_row.get("cep"), workflow_row.get("cep"), _first_non_null(itens_df, "cep"))
        ),
        "cidade_uf": _safe_str(
            _coalesce_value(
        ...
        "n_fases": n_fases,
        "custo_disp": custo_disp,
        "origem": "info_clientes" if uc_cadastrada else "parse",
        "uc_cadastrada": uc_cadastrada,
    }

    campos_pendentes: List[str] = []
    if not uc_cadastrada:
        campos_pendentes.append("unidade_consumidora")
    if cadastro.get("desconto_contratado") is None:
        campos_pendentes.append("desconto_contratado")
    if cadastro.get("custo_disp") is None:
        campos_pendentes.append("custo_disp")
    if not _safe_str(cadastro.get("status")):
        campos_pendentes.append("status")

    cadastro_minimo_completo = len(campos_pendentes) == 0
    elegivel_para_calculo = cadastro_minimo_completo and cadastro.get("status") == "Ativo"

    motivo_bloqueio: Optional[str] = None
    if not uc_cadastrada:
        motivo_bloqueio = "UC ainda nao cadastrada em info_clientes. Salve a revisao para concluir o cadastro minimo."
    elif campos_pendentes:
        labels = [CADASTRO_REQUIRED_FIELDS.get(field, field) for field in campos_pendentes]
        motivo_bloqueio = f"Campos pendentes no cadastro minimo: {', '.join(labels)}."
    elif cadastro.get("status") != "Ativo":
        motivo_bloqueio = "Status da UC precisa ser 'Ativo' para calcular."

    cadastro["campos_pendentes"] = campos_pendentes
    cadastro["cadastro_minimo_completo"] = cadastro_minimo_completo
    cadastro["elegivel_para_calculo"] = elegivel_para_calculo
    cadastro["motivo_bloqueio"] = motivo_bloqueio
    return cadastro
    ...
    detalhe.update(
        {
            ...
            "motivo_bloqueio": cadastro_cliente.get("motivo_bloqueio"),
...
def _append_observacao(current: Any, message: str) -> str:
    atual = _safe_str(current)
    novo = _safe_str(message) or ""
    if not atual:
        return novo
    if not novo:
        return atual
...
def _ensure_fatura_ready_for_calc(
    ...
    if _safe_str(workflow_row.get("status_parse")) == "erro_parse":
        raise HTTPException(status_code=400, detail="Nao e possivel calcular uma fatura com erro de parse.")
    ...
    if not cadastro_cliente.get("cadastro_minimo_completo"):
        raise HTTPException(
            status_code=400,
            detail={
                "message": cadastro_cliente.get("motivo_bloqueio")
                or "Cadastro minimo incompleto para calcular a fatura.",
    ...
    if cadastro_cliente.get("status") != "Ativo":
        raise HTTPException(
            ...
            detail={
                "message": cadastro_cliente.get("motivo_bloqueio")
                or "Status da UC precisa ser 'Ativo' para calcular.",
                "fields": ["status"],
            },
        )


def _apply_updates_to_df(df: pd.DataFrame, field_map: Dict[str, str], updates: Dict[str, Any]) -> pd.DataFrame:
    if df is None or df.empty:
        return df
...
def _persist_cliente_from_review(
    ...
    unidade_consumidora = _safe_str(cadastro_cliente.get("unidade_consumidora"))
    ...
    client_payload = {
        ...
        "n_fases": _safe_int(cadastro_cliente.get("n_fases")),
        "custo_disp": _safe_float(cadastro_cliente.get("custo_disp"), default=None),
        "updated_at": _now_utc(),
    }
...
def _update_workflow_row(fatura_id: str, updates: Dict[str, Any]) -> dict:
    ...
    updated_df.loc[:, "updated_at"] = _now_utc()
...
def _sum_item_quantity(itens_df: pd.DataFrame, codigos: set[str]) -> float:
    if itens_df is None or itens_df.empty or "codigo" not in itens_df.columns:
        return 0.0
    if "quantidade_registrada" not in itens_df.columns:
        return 0.0
    ...
    return _safe_float(filtered["quantidade_registrada"].sum(min_count=1), default=0.0) or 0.0
...
def _build_client_df_for_calc(workflow_row: pd.Series, itens_df: pd.DataFrame, medidores_df: pd.DataFrame) -> pd.DataFrame:
    ...
    if client_df is None or client_df.empty:
        raise HTTPException(
            status_code=400,
            detail={
                "message": "UC ainda nao cadastrada em info_clientes. Salve a revisao antes de calcular.",
                "fields": ["unidade_consumidora"],
            },
        )

    base_df = client_df.head(1).copy()
    for key in [
        "unidade_consumidora",
        "cliente_numero",
        "nome",
        ...
        base_df.loc[:, key] = cadastro_cliente.get(key)
...
def _calculate_fatura_impl(fatura_id: str, usuario_validacao: str) -> dict:
    workflow_df = _get_workflow_df(fatura_id)
    workflow_row = workflow_df.iloc[0]
    itens_df = _get_itens_df(fatura_id)
    medidores_df = _get_medidores_df(fatura_id)
    ...
    try:
        ...
        _update_workflow_row(
            fatura_id,
            {
                ...
                "observacoes": _append_observacao(workflow_row.get("observacoes"), str(exc)),
            },
        )
        raise HTTPException(status_code=500, detail=f"Erro ao executar calculo da fatura: {exc}")
    ...
    if boleto_df is None or boleto_df.empty:
        ...
        raise HTTPException(
            status_code=400,
            detail={"message": detail, "fields": [], "missing_reason": calc_result.missing_reason},
        )

    boleto_df = boleto_df[boleto_df["numero"].astype(str) == str(fatura_id)].copy()
    if boleto_df.empty:
        detail = "O calculo nao retornou uma linha para a fatura solicitada."
        _update_workflow_row(
            fatura_id,
            {
                "status_calculo": "erro_calculo",
...
def revisar_fatura(fatura_id: str, payload: FaturaRevisaoRequestSchema):
    try:
...
async def parse_faturas(files: List[UploadFile] = File(...)) -> FaturasParseResponseSchema:
    ...
    salvar_auto_executado = all(entry.get("ok", False) for entry in persist_result.values())
    ...
    arquivos_com_erro = [
        FaturaParseErroArquivoSchema(arquivo=r.arquivo, erros=list(r.erros or []))
        for r in resultados
        if not r.sucesso
    ]
